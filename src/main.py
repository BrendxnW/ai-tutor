...
@app.post("/api/canvas/courses/{course_id}/index")
async def start_canvas_course_index_api(course_id: str, request: Request):
    ...
    try:
        if await asyncio.to_thread(pinecone_namespace_has_records, namespace):
            ...
            CANVAS_COURSE_INDEX_STATUSES[namespace] = status
            CANVAS_COURSE_INDEX_TASKS[namespace] = asyncio.create_task(
    ...
    existing_status = CANVAS_COURSE_INDEX_STATUSES.get(namespace)
    existing_task = CANVAS_COURSE_INDEX_TASKS.get(namespace)
...
@app.get("/api/canvas/courses/{course_id}/index/status")
async def get_canvas_course_index_status_api(course_id: str, request: Request):
    ...
    namespace = build_canvas_course_namespace(username, course)
    status = CANVAS_COURSE_INDEX_STATUSES.get(namespace)
    if status:
        return status
...
async def index_canvas_course_modules_metadata(
    ...
    try:
        ...
        CANVAS_COURSE_INDEX_STATUSES[namespace] = make_canvas_index_status(
            final_status,
            namespace,
            course,
...
def upsert_records_to_pinecone(records, namespace):
    ...
    for start in range(0, len(records), PINECONE_UPSERT_BATCH_SIZE):
        batch = [
            ...
            for record in records[start : start + PINECONE_UPSERT_BATCH_SIZE]
        ]
