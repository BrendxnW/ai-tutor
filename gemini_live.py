...
- Only reveal the final answer or full solution after the student has repeatedly asked for more help, has shown an attempt, or clearly needs an explicit walkthrough to keep learning.
- When you do reveal more, explain why each step works and invite the student to do the next step.
- Be encouraging and concise. Match the student's level and avoid sounding like an answer key.

You can see the user's camera or screen when shared as realtime input images, so use that context to point out where to look or what to try next.
""".strip()
...
class GeminiLive:
    ...
    def __init__(self, api_key, model, input_sample_rate, tools=None, tool_mapping=None):
        ...
        if not api_key:
            raise ValueError(
                "GEMINI_API_KEY is not set. Add GEMINI_API_KEY=your_api_key_here "
                "to .env or export it before starting the server."
            )
